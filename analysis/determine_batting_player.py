...
def __get_defensive_formulas__(players: List[SingleLineStatsPlayer], pos_num: int):
    ...
    arm_runs_analysis = perform_regression(
        eligible_players,
        arm_runs_ram
    )
    ...
    zr_analysis = perform_regression(
        eligible_players,
        zr_ram
    )
    ...
    if pos_num == 2:
        ...
        frame_runs_analysis = perform_regression(
            eligible_players,
            frame_runs_ram
        )
...
def get_batting_player_formulas(players: List[SingleLineStatsPlayer]) -> BattingPlayerFormulas:
    ...
    k_analysis = perform_bats_handedness_regression(batting_only, k_ram)
    ...
    hr_analysis = perform_bats_handedness_regression(batting_only, hr_ram)
    ...
    walks_analysis = perform_bats_handedness_regression(batting_only, walks_ram)
    ...
    hits_analysis = perform_bats_handedness_regression(batting_only, hits_ram)
    ...
    xbh_analysis = perform_bats_handedness_regression(batting_only, xbh_ram)
    ...
    triple_rate_analysis = perform_regression(batting_only, triple_rate_ram)
    ...
    sba_analysis = perform_regression(batting_only, sba_ram)
    ...
    steal_analysis = perform_regression(batting_only, steal_ram)
